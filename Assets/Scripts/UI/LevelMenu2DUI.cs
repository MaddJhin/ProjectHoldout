...
/// </summary>
public class LevelMenu2DUI : Singleton<LevelMenu2D> {
	...
	public void createMenu ()
	{
		...
		if (showNextBackButtons == true)
		{
			if (nextButtonObject != null && backButtonObject != null)
			{
				...
				nextButtonObject.GetComponent<Canvas>().sortingOrder = itemsList.Count;
				backButtonObject.GetComponent<Canvas>().sortingOrder = itemsList.Count;
			}
		}
...
//						itemOffset.x = itemsList[i-1].GetComponent<Rect>().width + spacingBetweenItems;
//					else if (orientation == MenuOrientation.Vertical)
//						itemOffset.y = itemsList[i-1].GetComponent<Rect>().height + spacingBetweenItems;
//				}
				changePos.x += itemOffset.x;
				changePos.y += itemOffset.y;
				itemsList[i].transform.position = changePos;
				if (shouldIncreaseOrder)
					itemsList[i].GetComponent<Canvas>().sortingOrder = itemsList[i-1].GetComponent<Canvas>().sortingOrder + 1;
				else 
					itemsList[i].GetComponent<Canvas>().sortingOrder = itemsList[i-1].GetComponent<Canvas>().sortingOrder - 1;
			}
		}
	...
	public void sortOrderOfItems()
	{
		...
		{
			itemsList[i].GetComponent<Canvas>().sortingOrder = i-CurrentItem;
			//itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
		}
		for (int i=itemsList.Count-1; i>CurrentItem; i--)
		{
			itemsList[i].GetComponent<Canvas>().sortingOrder = CurrentItem-i;
			//itemsList[i].GetComponent<ClickTouchScript>().enabled = true;
		}
		itemsList[CurrentItem].GetComponent<Canvas>().sortingOrder = 0;
	...
	{
		_isMoving = false;
		itemsList[CurrentItem].GetComponent<Canvas>().sortingOrder = itemsList.Count-1;
